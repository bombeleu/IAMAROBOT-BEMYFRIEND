...
/// 
/// \code
/// DebugConsoleMenu.Log( "Hello World!");
/// 
/// // Now open it
...
/// 
/// \code
/// DebugConsoleMenu.RegisterCommand( "ShowFPS", ShowFPSCommand);
/// \endcode
/// 
...
/// myWatchInt.Value = 23;
/// 
/// DebugConsoleMenu.RegisterWatchVar(myWatchInt.Name, myWatchInt);
/// \endcode
/// 
...
/// </summary>
public class DebugConsoleMenu : MonoBehaviour
{
    private const float VERSION = 2.0F;
	private int s_width = Screen.width;
	private int s_height = Screen.height;
    ...
    {
        get { return DebugConsoleMenu.Instance._isOpen; }
        set { DebugConsoleMenu.Instance._isOpen = value; }
    }
    ...
    /// reference (which you do in mose cases), use DebugConsoleMenu.Instance and the required
    /// GameObject initialization will be done for you.
    /// </summary>
    public static DebugConsoleMenu Instance
    {
        get
        {
            if ( _instance == null )
            {
                _instance = FindObjectOfType( typeof( DebugConsoleMenu ) ) as DebugConsoleMenu;
                if ( _instance == null )
                {
                    GameObject console = new GameObject();
                    console.AddComponent( "DebugConsoleMenu" );
    ...
    private static DebugConsoleMenu _instance = null;
    ...
    void Awake()
    {
        ...
        _windowRect = new Rect( 20.0F, 20.0F, (s_width / 3.0F), (s_height*0.95f) );
        _cmdTable = new Hashtable();
        _watchVarTable = new Hashtable();      
        _displayString = new StringBuilder();
		
	/*
        _messages.Add( new Message( "/BOOT OK ", MessageTypes.System ) );
        _messages.Add( new Message( "/RAM OK ", MessageTypes.System ) );
        _messages.Add( new Message( "/OS Initialised ", MessageTypes.System ) );
        _messages.Add( new Message( "/////STARTING///// " ) );
      */        
       // this.RegisterCommandCallback( "close", CMDClose );
       // this.RegisterCommandCallback( "clear", CMDClear );
		this.RegisterCommandCallback( "Login", Login );
		//DEBUG AND TESTING COMMANDS (PUT A LOCK ON THESE IN FINAL GAME)
		/*this.RegisterCommandCallback( "GenSmallTest" , CMDGenSmallTestLevel);
		this.RegisterCommandCallback( "GenLargeTest" , CMDGenLargeTestLevel);*/
    ...
    void OnGUI()
    {
        ...
        if ( _isOpen )
        {
            _windowRect = GUI.Window( -1111, _windowRect, MainWindow, "M.A.I.N.T.A.I.N Console" );
            GUI.BringWindowToFront( -1111 );
			s_width = Screen.width;
			s_height = Screen.height;
			_windowRect.width = (s_width / 4.0f);
			_windowRect.height = (s_height/2.5f);
			_windowRect.y = s_height/2;
        }
    }
    ...
    {
        DebugConsoleMenu.Instance.LogMessage( text );
    }
    ...
    {
        DebugConsoleMenu.Instance.LogMessage( text, messageType );
    }
    ...
    {
        DebugConsoleMenu.Instance.LogMessage( text, displayColor );
    }
    ...
    {
        DebugConsoleMenu.Instance.LogMessage( text, messageType, displayColor, useCustomColor );
    }
    ...
    {
        DebugConsoleMenu.Instance.LogMessage( text, MessageTypes.Warning );
    }
    ...
    {
        DebugConsoleMenu.Instance.LogMessage( text, MessageTypes.Error );
    }
    ...
    {
        DebugConsoleMenu.Instance.ClearLog();
    }
    ...
    {
        DebugConsoleMenu.Instance.RegisterCommandCallback( commandString, commandCallback );
    }
    ...
    {
        DebugConsoleMenu.Instance.UnRegisterCommandCallback( commandString );
    }
...
/*    public static void RegisterWatchVar( string name, WatchVar watchVar )
    {
        DebugConsoleMenu.Instance.AddWatchVarToTable( name, watchVar );
    }
    ...
    {
        DebugConsoleMenu.Instance.RemoveWatchVarFromTable( name );
    }
	 */
	...
	public void Login(params string[] args)
	{
		LogMessage("Welcome " + args[0]);
	
		//SHOW MORE GUI, RIGHT SIDE
		//use a bool and on gui for now
	}
	
	public void CMDHelp( params string[] args)
	{
		LogMessage("Commands: Clear,Help,MoveForward,MoveBackward,TurnLeft,TurnRight,Print");
		
	}
	
	public void CMDPrint( params string[] args)
	{
...
public abstract class WatchVar
{
    ...
    {
        this._name = name;
        DebugConsoleMenu.RegisterWatchVar( _name, this );
    }
